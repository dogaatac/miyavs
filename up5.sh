...
while True:
    try:
        ...
        command = f'rclone move /mnt/up5/ "{ac_name}": --progress --log-file /root/rclone.log --config /root/.config/rclone/yolla.conf --drive-upload-cutoff 700G  --tpslimit 3 --drive-stop-on-upload-limit --drive-chunk-size 128M --no-traverse --retries 1 --ignore-existing --log-level INFO --drive-service-account-file "/root/.config/rclone/accounts/{selected_json_file}" -P'
