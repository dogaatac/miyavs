...
###
# TEK SEFERLİK İŞLEM:
#  => .fpt bul, chunk upload yap, bitince çık.
###

# 1) /mnt/pw/ => ilk .fpt'yi bul
fpt_file=$(find "$WATCH_DIR" -maxdepth 1 -type f -name "*.fpt" | head -n1)
if [[ -z "$fpt_file" ]]; then
    echo "[INFO] Yeni .fpt bulunamadı. Çıkıyorum..."
    exit 0
fi
...
rc=$?
if [[ $rc -ne 0 ]]; then
    echo "[ERR] chunker hata, kod=$rc. Çıkıyorum..."
    exit 1
fi

# 3) chunk dosyalarını bul
...
# 5) chunk i => SA[i % 6]. Son part => önce pointer, sonra part
total=${#chunk_files[@]}
...
echo "[OK] Tüm chunk + pointer yüklendi. Script kapanıyor..."
exit 0
