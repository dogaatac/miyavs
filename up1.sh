...
while True:
    try:
        ...
        command = f'rclone move /mnt/up1/ "{ac_name}": --log-file /root/rclone.log --progress --retries 1 --config /root/.config/rclone/yolla.conf  --tpslimit 3 --drive-stop-on-upload-limit --drive-chunk-size 128M --retries 1 --no-traverse --ignore-existing --log-level INFO --drive-service-account-file "/root/.config/rclone/accounts/{selected_json_file}" -P'
