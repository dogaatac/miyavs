...
while True:
    try:
        ...
        command = f'rclone move /mnt/up2/ "{ac_name}": --progress --config /root/.config/rclone/yolla.conf --drive-upload-cutoff 700G --multi-thread-streams 8 --tpslimit 3 --drive-stop-on-upload-limit --drive-chunk-size 256M --no-traverse --ignore-existing --log-level INFO --drive-service-account-file "/root/.config/rclone/accounts/{selected_json_file}" -P'
